...
const ADMIN_LEVEL_OPTIONS = [
  ...
  { value: 'elementary',       label: 'Elementary — A1–A2' },
  { value: 'pre_intermediate', label: 'Pre-Intermediate — A2–B1' },
  { value: 'intermediate',     label: 'Intermediate — B1–B2' },
  { value: 'advanced',         label: 'Advanced — B2–C1'  },
]

// New collections + legacy collections kept for backward compatibility
const COLLECTIONS = [
  'beginnerTests', 'elementaryTests', 'preIntermediateTests',
  'intermediateTests', 'advancedTests',
  'tests',
  'a1Tests', 'a2Tests', 'b1Tests', 'b2Tests', 'c1Tests', 'c2Tests',
]

const LEVEL_KEY_MAP = {
  beginnerTests:        'beginner',
  elementaryTests:      'elementary',
  preIntermediateTests: 'pre_intermediate',
  intermediateTests:    'intermediate',
  advancedTests:        'advanced',
  tests:                'intermediate',
  a1Tests:              'beginner',
  a2Tests:              'elementary',
  b1Tests:              'intermediate',
  b2Tests:              'pre_intermediate',
  c1Tests:              'advanced',
  c2Tests:              'advanced',
}

const LEVEL_CONFIG = {
  beginner:         { label: 'A0–A1', name: 'Beginner',         color: { background: 'rgba(16,185,129,0.2)',  color: '#6ee7b7', border: '1px solid rgba(16,185,129,0.3)'  } },
  elementary:       { label: 'A1–A2', name: 'Elementary',       color: { background: 'rgba(6,182,212,0.2)',   color: '#67e8f9', border: '1px solid rgba(6,182,212,0.3)'   } },
  pre_intermediate: { label: 'A2–B1', name: 'Pre-Intermediate', color: { background: 'rgba(99,102,241,0.2)',  color: '#a5b4fc', border: '1px solid rgba(99,102,241,0.3)'  } },
  intermediate:     { label: 'B1–B2', name: 'Intermediate',     color: { background: 'rgba(139,92,246,0.2)',  color: '#c4b5fd', border: '1px solid rgba(139,92,246,0.3)'  } },
  advanced:         { label: 'B2–C1', name: 'Advanced',         color: { background: 'rgba(244,63,94,0.2)',   color: '#fda4af', border: '1px solid rgba(244,63,94,0.3)'   } },
}
...
// Normalise the `level` field value stored inside a document.
// TestCreatorPage writes e.g. 'Pre_Intermediate'; admin filters use 'pre_intermediate'.
const DOC_LEVEL_NORMALIZE = {
  beginner:         'beginner',
...
// Prefer the document's own `level` field (TestCreatorPage sets it).
// Fall back to collection-based mapping (AddTestPage uses level-specific collections
// but does not write a `level` field into the document).
const levelKeyOf = (col, docLevel) => {
  if (docLevel && DOC_LEVEL_NORMALIZE[docLevel]) return DOC_LEVEL_NORMALIZE[docLevel]
  return LEVEL_KEY_MAP[col] ?? 'intermediate'
}
        ...
        try {
          ...
          const testsFromCollection = querySnapshot.docs.map(doc => {
            ...
            return {
              id: doc.id,
              collectionName,
              levelKey: levelKeyOf(collectionName, data.level),
              ...data,
            }
          })
