...
function getDisplayAnswer(question, answer, emptyLabel) {
  if (answer === undefined || answer === null) return emptyLabel
  if (!question.type) {
  ...
  if (question.type === 'word_order') {
    if (!Array.isArray(answer) || !answer.length) return emptyLabel
    return answer.map(i => question.scrambled_words[i]).join(' ')
  }
  return String(answer) || emptyLabel
}
...
export default function TestResultPage() {
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 flex items-center justify-center px-4">
        <div className="text-center">
          <h2 className="text-2xl font-bold text-white mb-2">{t('testResult.noResults')}</h2>
          <p className="text-slate-400 mb-6">{t('testResult.noResultsDesc')}</p>
          <button
            onClick={() => navigate('/level')}
            className="px-6 py-3 rounded-xl bg-indigo-500 text-white font-semibold hover:bg-indigo-600 transition-colors"
          >
            {t('testResult.goToTests')}
          </button>
        </div>
      </div>
    )
  ...
  const getScoreBg = (p) => {
    if (p >= 80) return 'bg-emerald-500/20 border-emerald-500/30'
    if (p >= 60) return 'bg-blue-500/20 border-blue-500/30'
  ...
  const handleRetakeTest = () => {
    if (testId === 'practice') {
      navigate('/tests/practice', { state: { levelId: level, testTitle, questions } })
    } else {
      navigate(`/tests/${testId}`, { state: { levelId: level } })
    }
  }
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 py-8 px-4">
      <div className="max-w-4xl mx-auto">
        <div className="mb-8">
          ...
          >
            <ArrowLeft className="w-4 h-4" /> {t('testResult.backToLevels')}
          </button>
          <div className="text-center">
            <h1 className="text-3xl sm:text-4xl font-bold text-white mb-2">
              Test <span className="gradient-text">{t('testResult.resultsTitle')}</span>
            </h1>
            <p className="text-gray-400">{testTitle || 'Test'}</p>
            <p className="text-sm text-slate-500 mt-1">{t('testResult.level')}: {level?.toUpperCase()}</p>
          </div>
        </div>
          ...
          <div className="text-center">
            ...
            <div className="grid grid-cols-3 gap-4 mb-6">
              ...
              <div className="bg-emerald-500/10 rounded-xl p-4 border border-emerald-500/30">
                ...
                <div className="text-sm text-gray-400">{t('testResult.correct')}</div>
              </div>
              <div className="bg-red-500/10 rounded-xl p-4 border border-red-500/30">
                <div className="text-2xl font-bold text-red-400">{incorrectCount}</div>
                <div className="text-sm text-gray-400">{t('testResult.incorrect')}</div>
              </div>
            </div>
            ...
            <div className="flex gap-3 justify-center">
              ...
              >
                <Home className="w-4 h-4" /> {t('testResult.tests')}
              </button>
              <button
              ...
              >
                <RotateCcw className="w-4 h-4" /> {t('testResult.retake')}
              </button>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-4">
          <h2 className="text-xl font-bold text-white mb-4">{t('testResult.details')}</h2>
          ...
          {questions.map((question, index) => {
            ...
            return (
              ...
              >
                <div className="p-5 cursor-pointer flex items-start gap-4" onClick={() => toggleQuestion(index)}>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <span className={`text-xs px-2 py-1 rounded-full font-semibold ${
                        ...
                        {isCorrect ? t('testResult.correctLabel') : t('testResult.incorrectLabel')}
                      </span>
                    </div>
                          ...
                          return (
                            <div key={optIndex} className={optionClass}>
                              <div className="flex items-center gap-3">
                                ...
                                {isCorrectOption && (
                                  <span className="ml-auto text-xs font-semibold text-emerald-400">{t('testResult.correctAnswer')}</span>
                                )}
                                {isSelected && !isCorrect && (
                                  <span className="ml-auto text-xs font-semibold text-red-400">{t('testResult.yourAnswer')}</span>
                                )}
                              </div>
                            </div>
                          )
                    ...
                    {isNewFmt && !question.options && (
                      <div className="space-y-2">
                        <div className={`p-3 rounded-xl border text-sm ${
                          ...
                          <span className="text-slate-400 text-xs">{t('testResult.yourAnswerLabel')}</span>
                          <span className={`font-semibold ${isCorrect ? 'text-emerald-300' : 'text-red-300'}`}>
                            {getDisplayAnswer(question, selectedAnswer, t('testResult.empty'))}
                          </span>
                        </div>
                        {!isCorrect && (
                    ...
                    {!isNewFmt && !isCorrect && (
                      <div className="mt-4 p-3 rounded-lg bg-blue-500/10 border border-blue-500/30">
                        <p className="text-sm text-blue-300">
                          <span className="font-semibold">{t('testResult.explanation')}</span>{' '}
                          <span className="font-bold">{LETTERS[question.correctAnswer]}</span>.
                        </p>
                      </div>
                    )}
                  </div>
